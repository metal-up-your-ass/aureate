...
Aureate needs a cross-platform audio plugin framework that can target AU and VST3 on macOS and VST3 on Windows, ship a Standalone build, and support a modern C++ toolchain. Which framework should the project build on?
...
* Breadth of built-in DSP coverage (filters, oversampling, convolution, dynamics, etc.) so the oversampled saturation and console-style tone shaping work isn't reinventing well-trodden primitives.
...
* Good, because `juce::dsp` provides ready building blocks (IIR/FIR filters, oversampling, convolution, state variable filters) that map directly onto this project's oversampled saturation and console-style tone shaping needs.
