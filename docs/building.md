...
Built plugin formats (AU, VST3, Standalone) land under `build/Aureate_artefacts/`, in a `Release/` subdirectory for multi-config generators (e.g. the Windows Visual Studio generator) or directly under `Aureate_artefacts/` for single-config generators (e.g. Ninja), depending on generator and configuration.
